...
| Tests offline `pytest -q` | 110 tests aprobados |
...
| Contenido del paquete | 23 archivos de la lista cerrada; sin sesión, token, cachés, pruebas ni claves de firma |
| Firma de desarrollo | Copia separada `-dev-signed.mcpb`; firma PKCS#7 verificada criptográficamente |
| Runtime extraído del MCPB 0.3.0 | `uv --frozen`, handshake y descubrimiento correctos desde un directorio limpio |
...
| Asistente 0.3.0 | Diseño autocontenido, CSP, aviso no oficial, lenguaje neutral y ausencia de recursos remotos comprobados; instalación final pendiente |
