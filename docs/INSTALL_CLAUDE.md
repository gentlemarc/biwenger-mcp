...
1. Descarga `biwenger-mcp-0.3.0.mcpb` y comprueba el SHA-256 publicado junto al archivo.
...
4. En la página local que se abre, escribe el correo y la contraseña propia de Biwenger. Si la cuenta se creó con Google, Apple o Facebook y no tiene contraseña propia, establécela mediante la recuperación oficial de Biwenger. No introduzcas la contraseña de esos proveedores.
5. Elige una liga compatible. Se admiten LaLiga, fichajes Clásica y los sistemas predefinidos Diario AS, SofaScore, Estadísticas, Feeberse, las dos medias y Biwenger Social.
6. Cierra completamente Claude Desktop y vuelve a abrirlo. Después llama a `get_context` y comprueba la liga y la puntuación.
